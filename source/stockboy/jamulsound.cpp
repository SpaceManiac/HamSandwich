...
int soundNumber;
static int sndVolume,musVolume;
...
void JamulSoundRegisterWindow(void)
{
	GM_registerMainWindow(dsoundHwnd);
}
...
void JamulSoundExit(void)
{
	...
	//UnregisterClass("dsoundwindow",hInst);
}
	...
	if(flags&SND_ONE)
	{
		for(i=0;i<MAX_SOUNDS_AT_ONCE;i++)
			if(playBuffer[i].soundNum==num)
			{
				...
				{
					playBuffer[i].seqNum=255;
					playBuffer[i].pan=pan;
					playBuffer[i].vol=vol;
					playBuffer[i].flags=flags|SND_PLAYING;
					playBuffer[i].priority=priority;
					JamulSoundPlay(playBuffer[i].dsHandle,playBuffer[i].pan,playBuffer[i].vol,SOUND_CUTOFF|playFlags);
					return;	// good job
				}
				else
