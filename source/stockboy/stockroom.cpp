...
void InitStockroomMenu(void)
{
	...
	pickMode=0;
	if(month!=2 || !smarch)
		InitStars(profile.starGot[GAME_STOCKROOM][month*4+week],0);
	else
		InitStars(profile.starGot[GAME_STOCKROOM][12*4+week],0);
	moUpClock=0;
}

void ExitStockroomMenu(void)
{
	delete blowSpr;
	free(backScr);
	SaveOptions();
...
void UpdateBackgd(void)
{
	...
	if(month==255)
		return;	// ignore month of 255, leave last backgd up
...
void ToggleSmarch(void)
{
	smarch=1-smarch;
	if(smarch)
		strcpy(monthName[2],"Smarch");
	else
		strcpy(monthName[2],"March");
}

byte UpdateStockroomMenu(int *lastTime,MGLDraw *mgl)
{
	char c;
	byte t;
	static byte fwClock;
			...
			case 2:	// really want to change high scores?
				...
				if(t&(CONTROL_B1|CONTROL_B2))
				{
					switch(cursor)
					{
						...
						case 1:	// clear them
							if(month==2 && smarch)
								ResetHighScore(GAME_STOCKROOM,12*4+week);
							else
								ResetHighScore(GAME_STOCKROOM,month*4+week);
							pickMode=1;
							cursor=2;
							MakeNormalSound(SND_MENUSELECT);
							break;
...
void RenderStockroomMenu(MGLDraw *mgl)
{
	char txt[4][32]={
		"Work This Week",
		"Pick Another Week",
	...
	for(i=0;i<4;i++)
	{
		for(j=0;j<3;j++)
		{
			...
			for(x=0;x<4;x++)
			{
				sg=profile.starGot[GAME_STOCKROOM][(i+j*4)*4+x];
				if((i+j*4)==2 && smarch)
					sg=profile.starGot[GAME_STOCKROOM][12*4+x];
				switch(sg)
				{
			...
			sg=profile.zodiac[i+j*4];
			if((i+j*4)==2 && smarch)
				sg=profile.zodiac[12];
			if(sg)
				RenderStar(5,4,(160*i+25)*FIXAMT,(88*j+14)*FIXAMT,mgl);
			sg=profile.empMonth[i+j*4];
			if((i+j*4)==2 && smarch)
				sg=profile.empMonth[12];
			if(sg)
	...
	if(month!=255)
	{
		...
		if(month!=2 || !smarch)
		{
			...
			RenderHighScore(GAME_STOCKROOM,month*4+week,1,500,315+50,mgl);
			RenderHighScore(GAME_STOCKROOM,month*4+week,0,500,315+50*2,mgl);
		}
		else
		{
			RenderHighScore(GAME_STOCKROOM,12*4+week,2,500,315,mgl);
			RenderHighScore(GAME_STOCKROOM,12*4+week,1,500,315+50,mgl);
			RenderHighScore(GAME_STOCKROOM,12*4+week,0,500,315+50*2,mgl);
		}
	}
	...
	switch(pickMode)
	{
		case 0:	// pick a week
			...
			PrintUnGlow(5,450,"Select a week to work in",0);
			break;
...
char *NameLevel(void)
{
	...
	sprintf(nameStr,"%s - Week %d",monthName[player.worldNum],player.levelNum+1);
	return nameStr;
}