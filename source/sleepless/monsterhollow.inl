...
void AI_Goodbone(Guy *me,Map *map,world_t *world,Guy *badguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		...
		if(me->seq==ANIM_ATTACK && me->frm==3 && me->reload==0 && badguy)
		{
			...
			{
				badguy->GetShot(Cosine(me->facing*32)*4,Sine(me->facing*32)*4,player.weaponLvl[WPN_BONEHEAD-1]*2,map,world);
			}
			me->reload=5;
		}
		if(me->seq==ANIM_A1 && me->frm==3 && me->reload==0 && badguy)
		{
			...
			a=me->facing*32-(player.weaponLvl[WPN_BONEHEAD-1]-1)*5;
			for(i=0;i<(player.weaponLvl[WPN_BONEHEAD-1]);i++)
			{
