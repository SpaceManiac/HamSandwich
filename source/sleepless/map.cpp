...
Map::Map(SDL_RWops *f)
{
	...
	if(count>0)
		SDL_RWread(f,badguy,count,sizeof(mapBadguy_t));
...
byte Map::Save(SDL_RWops *f)
{
	...
	if(count>0)
		SDL_RWwrite(f,badguy,count,sizeof(mapBadguy_t));
...
void Map::Update(byte mode,world_t *world)
{
	...
	for(i=0;i<width*height;i++)
	{
		...
		else if(mode==UPDATE_GAME)
		{
			...
			if(curWorld.terrain[map[i].floor].flags&TF_LAVA)
			{
				if(Random(50)==0)
					BlowSmoke(((i%width)*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,
								((i/width)*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,
								0,FIXAMT/2);
			}
			if(curWorld.terrain[map[i].floor].flags&TF_MUD)
			{
				if(Random(200)==0)
					StinkySteam(((i%width)*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,
								((i/width)*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,
								0,FIXAMT/2);
	...
	for(i=1;i<radius;i++)	// i is the radius of the square you are working with
	{
		for(cury=y-i;cury<=y+i;cury+=i*2)
			for(curx=x-i;curx<=x+i;curx++)
			{
				...
				{
					if(map[curx+cury*width].wall || (map[curx+cury*width].item>0 && (GetItem(map[curx+cury*width].item)->flags&IF_BULLETPROOF)))	// there's a wall here, so opaque
						map[curx+cury*width].opaque=1;
					else
...
byte Map::CompareRegions(int x,int y,int x2,int y2,int tx,int ty,byte checkMons)
{
	...
	for(i=x;i<=x2;i++)
		for(j=y;j<=y2;j++)
		{
			...
			if(m1->floor!=m2->floor || m1->wall!=m2->wall || m1->item!=m2->item)// || m1->light!=m2->light)  no light check in sleepless hollow!
				return 0;
		}
...
byte GlassSmashed(Map *map)
{
	...
	for(i=0;i<map->width*map->height;i++)
		if(map->map[i].floor==278)
			return 0;

	if(MonstersAlive(MONS_EVILCLONE))
		return 0;
