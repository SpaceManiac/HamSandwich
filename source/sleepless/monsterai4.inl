...
{
	Guy *g;
	byte summonTab[]={MONS_BEETLE,MONS_FROG2,MONS_BOOMKIN,MONS_SQUASH};
	byte timerTab[]={3,30*3,30*5,30*3};
	...
	if((!me->reload) && Random(20)==0 && RangeToTarget(me,goodguy)<600*FIXAMT)
	{
	...
	if(me->action==ACTION_BUSY)
	{
		...
		if(me->seq==ANIM_ATTACK && me->frm>6)
		{
			...
			if(me->frm==13 && me->frmTimer<64)
			{
				for(x=0;x<256;x+=8)
				{
					FireExactBullet(me->x,me->y,FIXAMT*10,Cosine(x)*8,Sine(x)*8,0,0,16,x,BLT_SPORE,me->friendly);
					FireExactBullet(me->x,me->y,FIXAMT*10,Cosine(x)*4,Sine(x)*4,0,0,16,x,BLT_SPORE,me->friendly);
				}
			}
		}
...
void AI_Beetle(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->seq==ANIM_MOVE && me->reload==0)
	{
		if(FindVictim(me->x>>FIXSHIFT,me->y>>FIXSHIFT,24,Cosine(me->facing*32)*4,Sine(me->facing*32)*4,1,map,world,me->friendly))
		{
			me->reload=4;
		}
	}
