...
void SetTagged(Guy *t)
{
	tagged=t;
}

void SetVictim(Guy *t)
{
	victim=t;
}
...
byte SpecialPaintPurple(int x,int y)
{
	int i,j;
	...
	for(i=0;i<MAX_SPECIAL;i++)
	{
		if(spcl[i].x==x && spcl[i].y==y)
		{
			for(j=0;j<NUM_TRIGGERS;j++)
				if(spcl[i].trigger[j].type==TRG_MONSCOLOR && spcl[i].trigger[j].value2==6)
					return 1;
		}
	}
	return 0;
...
void FindSpecialCandle(int myx,int myy)
{
	int i,j;
	int s;
	...
	{
		for(s=0;s<5;s++)
			if(spcl[i].effect[s].type==EFF_ITEM && spcl[i].effect[s].value==ITM_CANDLE && spcl[i].x!=255)
			{
				if(((myx-spcl[i].effect[s].x)*(myx-spcl[i].effect[s].x)+(myy-spcl[i].effect[s].y)*(myy-spcl[i].effect[s].y))<j)
				{
					player.candleX=spcl[i].effect[s].x;
					player.candleY=spcl[i].effect[s].y;
					j=(myx-player.candleX)*(myx-player.candleX)+(myy-player.candleY)*(myy-player.candleY);
				}
			}
	}
