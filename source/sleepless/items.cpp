...
item_t baseItems[]={
	...
	{"Turrets",-4,-5,68,0,0,0,
		...
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Turrets!",SND_WEAPON},
	{"Mind Control Ray",0,0,69,0,0,0,
		5,
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Mind Control Ray!",SND_WEAPON},
	{"Reflect Shield",-3,-3,67,0,0,0,
		5,
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Reflector Shield!",SND_WEAPON},
	...
	{"Jetpack",0,0,71,0,0,0,
		...
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Jetpack!",SND_WEAPON},
	...
	{"Swapgun",0,-3,70,0,0,0,
		...
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Swapgun!",SND_WEAPON},
	...
	{"Scanner",0,0,113,0,0,0,
		...
		IF_PICKUP|IF_SHADOW,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Monster Scanner!",SND_WEAPON},
	{"Mini-Sub",0,0,104,0,0,0,
		0,
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Mini-Sub!",SND_SUBSTART},
	...
	{"Freeze Ray",0,-4,112,0,0,0,
		...
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Freeze Ray!",SND_WEAPON},
	...
	{"Stopwatch",0,-3,129,0,0,0,
		...
		IF_PICKUP,
		IT_PICKUP|IT_WEAPON,
		ITR_GET,IE_WEAPON,WPN_NONE,"Stopwatch!",SND_WEAPON},
...
void RenderItem(int x,int y,byte type,char bright,byte flags)
{
	...
	else
	{
		...
		if(items[type].effect==IE_WEAPON && !(items[type].flags & IF_USERJSP))
		{
...
byte AppendItems(SDL_IOStream *f)
{
	int i;
	word changedItems;
	byte getNumber;
...
byte InteractWithItem(Guy *me,mapTile_t *m,int x,int y)
{
	...
	if((me->aiType==MONS_MINECART || me->aiType==MONS_YUGO) && (items[type].trigger&ITR_MINECART))
	{
		if(TriggerItem(me,m,x,y))
		{
			MakeCustomSound(items[type].sound,(x*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(y*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,SND_CUTOFF,1000);
...
byte BulletHitItem(byte bulType,mapTile_t *m,int x,int y)
{
	...
	if(bulType==BLT_HAMMER && (items[type].trigger&ITR_CHOP))
	{
		if(TriggerItem(NULL,m,x,y))
		{
			if(!editing && !player.cheated && verified)
