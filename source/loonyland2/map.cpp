...
void Map::ChopTree(int x,int y,byte fx)
{
	int i;

	map[x+y*width].item=IT_STUMP;
	map[x+(y-1)*width].item=IT_TREEXING;

	for(i=0;i<4;i++)
	{
		map[x+(y-2-i)*width].item=IT_INVISOWALK;
		if(fx)
			ParticleBoom(PART_FLOATYPOP,(x*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,((y-2-i)*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,FIXAMT*20);
	}
}
...
void Map::Init(world_t *wrld)
{
	...
	for(i=0;i<width*height;i++)
	{
		...
		if(map[i].item==IT_TOOLBOX && (GotItem(IT_TOOLBOX)>0 || player.var[VAR_QUESTDONE+QUEST_HANDY]))
			map[i].item=IT_NONE;
		if(map[i].item==IT_PRESENT && player.var[VAR_PRESENT+map[i].tag])
			map[i].item=IT_NONE;	// remove present you've already gotten
		if(player.levelNum==LVL_AXESRUS && player.var[VAR_QUESTDONE+QUEST_AXES]==0 && (map[i].item>=IT_AXE && map[i].item<=IT_GOLDENAXE))
			map[i].item=IT_NONE;
		if(map[i].item>=IT_CHEST1 && map[i].item<=IT_CHEST5 && player.var[VAR_CHEST+map[i].tag])
		{
			// already been opened
			map[i].itemInfo=10;
		}
		if(map[i].item==IT_BURIEDSTONE && (GotItem(IT_SHARPSTONE) || player.var[VAR_QUESTDONE+QUEST_SHARPEN]))
		{
			map[i].item=IT_NONE;
			map[i].light=0;
		}
		if(map[i].item==IT_THING && (GotItem(IT_THING) || player.var[VAR_FURNACE]))
			map[i].item=IT_NONE;
		if(map[i].item==IT_THING2 && (GotItem(IT_THING2) || player.var[VAR_FURNACE]))
			map[i].item=IT_NONE;
		if(map[i].item==IT_WATCH && (GotItem(IT_WATCH) || player.var[VAR_FURNACE]))
			map[i].item=IT_NONE;

		if((map[i].item==IT_ROCK2 || map[i].item==IT_CAVEROCK || map[i].item==IT_CAVEROCK2)&& (i%width)<width-1 && i<width*height-1 && map[i+1].item==IT_NONE)
			map[i+1].item=IT_INVISO;

		if((map[i].item==IT_SUPPLIES || map[i].item==IT_SUPPLIES2) && player.var[VAR_SNUGGLY]==0 && player.var[VAR_SUPPLIES]==20)
			map[i].item=IT_NONE;	// remove the supplies if you've destroyed them

		if(map[i].item==IT_CASTLESWITCH && player.levelNum==LVL_CASTLE1)
		{
			if(player.var[VAR_SWITCH+map[i].tag-1])
			{
				map[i].itemInfo=8;
				ZapWallTag(this,map[i].tag,map[i].floor);
			}
		}
		if(map[i].item==IT_CASTLESWITCH && player.levelNum==LVL_CASTLE2)
		{
			if(player.var[VAR_SWITCH2+map[i].tag-1])
			{
				map[i].itemInfo=8;
				ZapWallTag(this,map[i].tag,map[i].floor);
			}
		}
		if(player.levelNum==LVL_PIT && map[i].tag==1 && player.var[VAR_FURNACE])
		{
			if(map[i].item==IT_ICEBLOCK)
				map[i].item=IT_ICEMELT;
			else
				map[i].item=IT_NONE;
		}

		if(map[i].item==IT_CHOPTREE)
		{
			if(player.levelNum==LVL_LAKE)	// first tree
				j=VAR_TREECHOP;
			else
				j=VAR_TREECHOP+1;
			...
			if(player.var[j])
			{
				// it is chopped, bring it down
				ChopTree(i%width,i/width,0);
			}
		}
		if(map[i].item>=IT_KEY1 && map[i].item<=IT_KEY5 && GotItem(map[i].item))
			map[i].item=IT_NONE;
		if(player.var[VAR_GOURDDEAD] && map[i].item>=IT_VINEWALL && map[i].item<=IT_VINEWALL3)
			map[i].item=IT_DEADVINE+(rand()%2);
		if(map[i].item==IT_ENERGYBARRIER && player.var[VAR_BARRIER+map[i].tag])
			map[i].item=IT_NONE;
		if(map[i].item==IT_STUMP && player.var[VAR_ROPED+map[i].tag])
			RopeStump(this,i%width,i/width,0);
		if(player.levelNum==LVL_FOREST && map[i].tag==93 && player.var[VAR_YOUTH])
			map[i].item=IT_NONE;	// remove the shroom that became shrumford
		if(player.var[VAR_CAPTAIN2]!=3)
			player.var[VAR_CAPTAIN2]=0;
		if(player.levelNum==LVL_THRONE && player.var[VAR_SUPERTITAN] && map[i].tag==3)
			map[i].item=IT_NONE;
	}
	if(player.levelNum==LVL_GORGE && player.var[VAR_FROSTGATE])
		SmashFrostGate(this,0);

	if(player.levelNum==LVL_THRONE && player.var[VAR_SUPERTITAN] && GotItem(IT_KEY5)==0)
		map[32+57*width].item=IT_KEY5;
	...
	{
		// add in onion and thugs
		Guy *g;
		g=AddGuy((9*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(38*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,0,MONS_VILLAGER);
		if(g)
		{
			g->tag=24;	// Onion
			g->mind=1;
		...
		g=AddGuy((12*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(36*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,0,MONS_BRUISER);
		if(g)
			g->team=GOOD;
		g=AddGuy((12*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(40*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,0,MONS_BRUISER);
		if(g)
			g->team=GOOD;
		g=AddGuy((16*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(37*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,0,MONS_BRUISER);
		if(g)
			g->team=GOOD;
	}
	else if(player.levelNum==LVL_TOYFACTORY && player.var[VAR_QUESTDONE+QUEST_FACTORY2] && player.var[VAR_ONIONCHAT] &&
		player.var[VAR_SNUGGLY]==0 && GotItem(IT_KEY2)==0)
	{
		map[9+38*width].item=IT_KEY2;
	}
	if(player.var[VAR_PRISONBREAK]==1 && player.levelNum==LVL_ONIONCAMP)
		PrisonBreak(this,1);
	if(player.var[VAR_PRISONBREAK]==1 && player.levelNum==LVL_CLEARING)
		PrisonBreak(this,2);
	if(player.levelNum==LVL_ONIONCAMP && ((player.var[VAR_SNUGGLY]==0 && player.var[VAR_QUESTDONE+QUEST_CLOCKJUNK]) ||
		(player.var[VAR_SNUGGLY]==1 && player.var[VAR_JUNKDEAD])))
		map[14+20*width].item=IT_NONE;	// clear the block so you can get in the junkyard
	...
	for(i=0;i<width*height;i++)
	{
		...
		if(mode!=UPDATE_EDIT) // edit mode doesn't animate since it would screw it up
		{
			...
			if(timeToAnim==2)
			{
				...
				if(map[i].item>=IT_CHEST1 && map[i].item<=IT_CHEST5 && map[i].itemInfo>0 && map[i].itemInfo<10)
					map[i].itemInfo++;
				if(map[i].item>=IT_DOOR1 && map[i].item<=IT_DOOR5 && map[i].itemInfo>0)
				{
					if(map[i].itemInfo<8)
						map[i].itemInfo++;
					else
						map[i].item+=(IT_DOOROPEN1-IT_DOOR1);
				}
				if(map[i].item==IT_GEYSER)
					ParticleBoom(PART_STEAM,(x*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(y*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,30*FIXAMT);
				if(map[i].item==IT_UPGEYSER)
					ParticleBoom(PART_STEAM,(x*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(y*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,5*FIXAMT);
				if(map[i].item==IT_CASTLESWITCH && map[i].itemInfo>0)
				{
					if(map[i].itemInfo<8)
						map[i].itemInfo++;
					else if(player.levelNum==LVL_CASTLE1 && !player.var[VAR_SWITCH+map[i].tag-1])
					{
						PlayerSetVar(VAR_SWITCH+map[i].tag-1,1);
						ZapWallTag(this,map[i].tag,map[i].floor);
					}
					else if(player.levelNum==LVL_CASTLE2 && !player.var[VAR_SWITCH2+map[i].tag-1])
					{
						PlayerSetVar(VAR_SWITCH2+map[i].tag-1,1);
						ZapWallTag(this,map[i].tag,map[i].floor);
					}
				}
			}
			if(player.var[VAR_GOURDDEAD] && map[i].item>=IT_VINEWALL && map[i].item<=IT_VINEWALL3 && (rand()%1000)==0)
				map[i].item=IT_DEADVINE+map[i].item-IT_VINEWALL;	// randomly kill the vines over time
			if(mode!=UPDATE_FADE)
			{
				if(map[i].item>=IT_SHROOM && map[i].item<=IT_SHROOMPATCH2 && config.shading==1)
					BrightTorch(x,y,5,6);
				if(rand()%2 && (map[i].item>=IT_CRYSTAL && map[i].item<=IT_CRYSTAL4))
					BrightTorch(x,y,5,6);
			}
...
byte TempBrightCallback(int x,int y,int cx,int cy,int value,Map *map)
{
	...
	if(b<0)
		return 1;
	if(map->map[x+y*map->width].light>0)
...
byte TempNormalLightCallback(int x,int y,int cx,int cy,int value,Map *map)
{
	...
	if(map->map[x+y*map->width].templight<b)
		map->map[x+y*map->width].templight++;
	if(map->map[x+y*map->width].templight<b)
		map->map[x+y*map->width].templight++;
	if(map->map[x+y*map->width].templight<b)
		map->map[x+y*map->width].templight++;
	if(map->map[x+y*map->width].templight<b-6)
		map->map[x+y*map->width].templight+=4;
...
{
	LOS(x,y,15,4,TempNormalLightCallback);
}
...
void Map::Render(world_t *world,int camX,int camY,byte flags)
{
	...
	camX-=320;//+TILE_WIDTH*6;
	camY-=240;//+TILE_HEIGHT*6;

	tileX=(camX/TILE_WIDTH)-1-7;
	tileY=(camY/TILE_HEIGHT)-1-6;
	ofsX=camX%TILE_WIDTH;
	ofsY=camY%TILE_HEIGHT;

	scrX=-ofsX-TILE_WIDTH*(8);
	for(i=tileX;i<tileX+(SCRWID/TILE_WIDTH+7+8);i++)
	{
		scrY=-ofsY-TILE_HEIGHT*(7);
		for(j=tileY;j<tileY+(SCRHEI/TILE_HEIGHT+6+8);j++)
		{
				...
				else
				{
					...
					if(j<height-1)
					{
						...
						if((!map[i+(j+1)*width].wall) ||
							(world->terrain[map[i+(j+1)*width].floor].flags&TF_TRANS))
						{
							if(world->terrain[map[i+j*width].floor].flags&TF_WATERFX)
...
void Map::LoadFromProgress(SDL_IOStream *f)
{
	if(map)
		free(map);
...
void SpecialStepCheck(Map *map,int x,int y,Guy *me)
{
	...
	if(player.levelNum==LVL_CAPTAIN && player.var[VAR_SNUGGLY]==0 && player.var[VAR_QUESTDONE+QUEST_LEADER] && !player.var[VAR_QUESTASSIGN+QUEST_FURNACE])
		return;	// specials are disabled in the brief interval between killing the boss and hearing from the major, so as to avoid losing the quest
	...
	for(i=0;i<MAX_SPECIAL;i++)
	{
			...
			map->special[i].trigX==x && map->special[i].trigY==y)
		{
			if((me->type==player.monsType && (map->special[i].trigger&TRG_STEP)) ||
			   (me->team==EVIL && (map->special[i].trigger&TRG_ENEMYSTEP)))
			{
...
void CheckForChopTree(Map *map,int x,int y)
{
	...
	for(i=x-1;i<x+1;i++)
		for(j=y-1;j<y+1;j++)
		{
			if(i>=0 && j>=0 && i<map->width && j<map->height && map->map[i+j*map->width].item==IT_CHOPTREE)
			{
				if(player.axe.sharpened)
				{
					ExplodeParticles2(PART_DIRT,(i*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(j*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,FIXAMT*20,30,3);
					map->ChopTree(i,j,1);
					MakeNormalSound(SND_CHOPTREE);
					if(player.levelNum==LVL_LAKE)
						PlayerSetVar(VAR_TREECHOP,1);
					else
						PlayerSetVar(VAR_TREECHOP+1,1);
				}
				else
				{
					NoRepeatNewMessage("Your axe isn't sharp enough!",0);
					MakeNormalSound(SND_PARRY);
					ParticleBoom(PART_WHITEPOP,(i*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(j*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,FIXAMT*20);
				}
			}
		}
}
...
byte RopeStump(Map *map,int x,int y,byte fx)
{
	...
	if(x>7 && map->map[x-7+y*map->width].item==IT_STUMP && map->map[x-7+y*map->width].tag==map->map[x+y*map->width].tag)
	{
		// rope to the left
		for(i=x-6;i<x;i++)
		{
			map->map[i+y*map->width].item=IT_INVISOWALK;	// bridge
			if(fx)
				ParticleBoom(PART_FLOATYPOP,(i*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(y*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,FIXAMT*20);
		}
		map->map[x-7+y*map->width].item=IT_ROPEDSTUMP;
		map->map[x+y*map->width].item=IT_NONE;
		return 1;
	}
	else if(x<map->width-7 && map->map[x+7+y*map->width].item==IT_STUMP && map->map[x+7+y*map->width].tag==map->map[x+y*map->width].tag)
	{
		// rope to the right
		for(i=x+1;i<x+7;i++)
		{
		...
		map->map[x+y*map->width].item=IT_ROPEDSTUMP;
		map->map[x+7+y*map->width].item=IT_NONE;
		return 1;
	}
	return 0;
}
...
{
	if(x<0 || x>=curMap->width || y<0 || y>=curMap->height)
		return 0;

	if(curWorld.terrain[curMap->map[x+y*curMap->width].floor].flags&type)
	{
		if(type&(TF_SKY|TF_CLIFF|TF_WATER|TF_LAVA))
		{
			if(itemInfo[curMap->map[x+y*curMap->width].item].flags&IF_WALK)
				return 0;
		}
		return 1;
	}
...
void PrisonBreak(Map *map,byte situation)
{
	...
	{
		map->map[25+0*map->width].item=IT_EXIT;
		...
		for(i=0;i<MAX_SPECIAL;i++)
			if(map->special[i].trigger==0)
			{
				map->special[i].x=25;
				map->special[i].y=0;
				map->special[i].trigX=25;
				map->special[i].trigY=0;
				map->special[i].trigger=TRG_STEP;
				map->special[i].effect=SPC_GOTOMAP;
				map->special[i].value=LVL_CLEARING;
				map->special[i].effectX=14;
				map->special[i].effectY=32;
				break;
			}
		if(situation==0)	// initial break, during level
		{
			// also add in the guy
			Guy *g;
			g=AddGuy((26*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(3*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,0,MONS_BUNNYBOSS);
			if(g)
			{
				g->tag=30;	// Captain
				g->team=GOOD;
			}
			FireBullet(goodguy->ID,(25*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(1*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,0,0,BLT_KABOOM,0);
			MakeNormalSound(SND_BOOM);
			BeginChatting(30);
		}
	}
	if(situation==2)	// in the clearing, not the camp
	{
		map->map[14+34*map->width].item=IT_EXIT;
		...
		for(i=0;i<MAX_SPECIAL;i++)
			if(map->special[i].trigger==0)
			{
				map->special[i].x=14;
				map->special[i].y=34;
				map->special[i].trigX=14;
				map->special[i].trigY=34;
				map->special[i].trigger=TRG_STEP;
				map->special[i].effect=SPC_GOTOMAP;
				map->special[i].value=LVL_ONIONCAMP;
				map->special[i].effectX=25;
				map->special[i].effectY=2;
				break;
			}
		if(GotItem(IT_KEY3)==0)
		{
			map->map[14+30*map->width].item=IT_KEY3;
		}
	}
}
...
void SmashFrostGate(Map *map,byte fx)
{
	...
	player.var[VAR_FROSTGATE]=1;

	for(i=0;i<map->width*map->height;i++)
	{
		if(map->map[i].item==IT_ICEBLOCK)
		{
			map->map[i].item=IT_ICEMELT;
			if(fx)
			{
				for(j=0;j<5;j++)
				{
					ParticleBoom(PART_SNOW,((i%map->width)*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,((i/map->width)*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,FIXAMT*10);
					ParticleBoom(PART_WHITEPOP,((i%map->width)*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,((i/map->width)*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,FIXAMT*10);
				}
			}
		}
	}
...
byte DrawStones(Map *map,Guy *me,word dmg)
{
	...
	while(num>0)
	{
		tries++;
		if(tries==1000)
			return (num<3);	// couldn't find any spots to do it!  returns 0 only if a complete failure (so boss can do something else)
		x=rand()%(map->width);
		y=rand()%(map->height);

		if((map->map[x+y*map->width].tag==1 || map->map[x+y*map->width].tag==2) && (goodguy->mapx!=x && goodguy->mapy!=y))
		{
			map->map[x+y*map->width].templight=20;
			tries=0;
			num--;
			map->map[x+y*map->width].tag=0;
			if(y>0 && (map->map[x+(y-1)*map->width].floor==298 || map->map[x+(y-1)*map->width].floor==603))
				map->map[x+y*map->width].floor=603;	// black
			else
				map->map[x+y*map->width].floor=298;	// edge
			if(y<map->height-1 && map->map[x+(y+1)*map->width].floor==298)
				map->map[x+(y+1)*map->width].floor=603;	// if there was an edge below it, we need to make it a hole instead
			FireBullet(me->ID,(x*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(y*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,0,64,BLT_BLOCK,DamageAmt(dmg,me->team));
			MakeSound(SND_DRAWSTONE,(x*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(y*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,SND_CUTOFF|SND_RANDOM,1200);
			return 1;
		}
	}
	return 1;
...
void ExplodeStones(Map *map)
{
	...
	if(goodguy->mapx>=23 && goodguy->mapx<=41 && goodguy->mapy>=34 && goodguy->mapy<=40)
	{
		goodguy->z=FIXAMT*10;
		goodguy->dx=0;
		goodguy->dy=FIXAMT*10;
		goodguy->kbdx=0;
		goodguy->kbdy=FIXAMT*10;
		goodguy->kbdz=FIXAMT*10;
	}
	MakeNormalSound(SND_BOOM);
	for(x=0;x<map->width;x++)
		for(y=0;y<map->height;y++)
		{
			if(map->map[x+y*map->width].tag==2)
			{
				map->map[x+y*map->width].tag=0;
				if(y>0 && (map->map[x+(y-1)*map->width].floor==298 || map->map[x+(y-1)*map->width].floor==603))
					map->map[x+y*map->width].floor=603;	// black
				...
				if(y<map->height-1 && map->map[x+(y+1)*map->width].floor==298)
					map->map[x+(y+1)*map->width].floor=603;	// if there was an edge below it, we need to make it a hole instead
				b=FireBullet(goodguy->ID,(x*TILE_WIDTH+TILE_WIDTH/2)*FIXAMT,(y*TILE_HEIGHT+TILE_HEIGHT/2)*FIXAMT,(rand()%(FIXAMT*30)),(byte)(rand()%256),BLT_BLOCK4,0);
				if(b)
				{
					b->dy=FIXAMT*8-FIXAMT*3+(rand()%(FIXAMT*6+1));
					b->speed=FIXAMT*4-FIXAMT*2+(rand()%(FIXAMT*6));
				}
			}
		}
}
...
void DisableExits(Map *map)
{
	...
	for(i=0;i<map->width*map->height;i++)
	{
		if(map->map[i].item==IT_EXIT)
		{
			map->map[i].item=IT_NONE;
			for(j=0;j<MAX_SPECIAL;j++)
				if(map->special[j].trigX==(i%map->width) && map->special[j].trigY==(i/map->width))
					map->special[j].effect=0;
		}
	}
}
