...
static void ChooseTriggerClick(int id)
{
	if(rightClick)
	{
		spcl.trigger[id/100-1].type=TRG_NONE;
		MakeNormalSound(SND_MENUCLICK);
		return;
	}
	...
	selectY=(curTrig*38+30)-(14*MAX_TRIGGER)/2;
	if(selectY<0)
		selectY=0;
	if(selectY+14*MAX_TRIGGER>475)
		selectY=475-14*MAX_TRIGGER;
}
...
static void ChooseEffectClick(int id)
{
	if(rightClick)
	{
		spcl.effect[(id-ID_EFF0)/100].type=EFF_NONE;
		MakeNormalSound(SND_MENUCLICK);
		return;
	}
...
static void AndOrClick(int id)
{
	...
	spcl.trigger[t].flags^=TF_AND;
	if(spcl.trigger[t].flags&TF_AND)
...
static void SetupTriggerButtons(int t,int y)
{
	...
	// not button
	MakeButton(BTN_RADIO,ID_TRIG0+OFS_NOT+100*t,((spcl.trigger[t].flags&TF_NOT)!=0)*CHECK_ON,
	...
	// and/or button
	if(t<4)	// last one has no button for this
	{
		if(spcl.trigger[t].flags&TF_AND)
	...
	switch(spcl.trigger[t].type)
	{
		...
		case TRG_STEP:
			...
			sprintf(s,"%d",spcl.trigger[t].value2);
			...
			sprintf(s,"%d, %d",spcl.trigger[t].x,spcl.trigger[t].y);
		...
		case TRG_STEPRECT:
			...
			sprintf(s,"(%d,%d)-(%d,%d)",spcl.trigger[t].x,spcl.trigger[t].y,((word)spcl.trigger[t].value2)%256,((word)spcl.trigger[t].value2)/256);
		...
		case TRG_HAVEITEM:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+3+100*t,0,255,y+17,140,14,GetItem(spcl.trigger[t].value)->name,ItemClick);
			if(spcl.trigger[t].flags&TF_LESS)
		...
		case TRG_PASSLEVELS:
			...
			sprintf(s,"%d",spcl.trigger[t].value);
		...
		case TRG_PASSLEVEL:
			...
			sprintf(s,"%s",EditorGetWorld()->map[spcl.trigger[t].value]->name);
		...
		case TRG_TIMED:
			...
			sprintf(s,"%0.2f",(float)spcl.trigger[t].value/30.0f);
			...
			sprintf(s,"%0.2f",(float)spcl.trigger[t].value2/30.0f);
		...
		case TRG_MONSTER:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+3+100*t,0,270,y+17,140,14,MonsterName(spcl.trigger[t].value),MonsterClick);
			if(spcl.trigger[t].flags&TF_LESS)
		...
		case TRG_KILL:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,75,y+17,140,14,MonsterName(spcl.trigger[t].value),MonsterClick);
		...
		case TRG_VAR:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,120,y+17,30,14,VarName(spcl.trigger[t].value),VarClick);
		...
		case TRG_FLOOR:
			...
			sprintf(s,"%c%c",(char)(spcl.trigger[t].value%128),(char)(spcl.trigger[t].value/128));
		...
		case TRG_RANDOM:
			...
			sprintf(s,"%0.2f%%",(float)spcl.trigger[t].value/(float)FIXAMT);
		...
		case TRG_LIFE:
			MakeButton(BTN_STATIC,ID_TRIG0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"If",NULL);
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,60,y+17,140,14,MonsterName(spcl.trigger[t].value),MonsterClick);
			MakeButton(BTN_STATIC,ID_TRIG0+OFS_CUSTOM+2+100*t,0,204,y+17,1,1,"at",NULL);
			if(spcl.trigger[t].x==255)
				sprintf(s,"Anywhere");
			else
				sprintf(s,"%d, %d",spcl.trigger[t].x,spcl.trigger[t].y);
		...
		case TRG_STEPTILE:
			...
			sprintf(s,"%c%c",(char)(spcl.trigger[t].value2%128),(char)(spcl.trigger[t].value2/128));
		...
		case TRG_ITEM:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+3+100*t,0,250,y+17,140,14,GetItem(spcl.trigger[t].value)->name,ItemClick);
			break;
		case TRG_AWAKE:
			MakeButton(BTN_STATIC,ID_TRIG0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"If",NULL);
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,60,y+17,140,14,MonsterName(spcl.trigger[t].value),MonsterClick);
		...
		case TRG_COMPMAP:
			...
			sprintf(s,"(%d,%d)-(%d,%d)",((word)spcl.trigger[t].value)%256,((word)spcl.trigger[t].value)/256,
				(((word)spcl.trigger[t].value)%256)+((((word)spcl.trigger[t].value2)%256)-spcl.trigger[t].x),
				(((word)spcl.trigger[t].value)/256)+((((word)spcl.trigger[t].value2)/256)-spcl.trigger[t].y)
				);
		...
		case TRG_COMPVAR:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,120,y+17,30,14,VarName(spcl.trigger[t].value),VarClick);
			MakeButton(BTN_STATIC,ID_TRIG0+OFS_CUSTOM+2+100*t,0,154,y+17,1,1,"is",NULL);
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,169,y+17,30,14,VarName(spcl.trigger[t].value2),Var2Click);
			if(spcl.trigger[t].flags&TF_LESS)
				MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+4+100*t,0,213,y+17,80,14,"Or Less",LessMoreClick);
			else if(spcl.trigger[t].flags&TF_MORE)
				MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+4+100*t,0,213,y+17,80,14,"Or More",LessMoreClick);
		...
		case TRG_MONSINRECT:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,90,y+17,140,14,MonsterName(spcl.trigger[t].value),MonsterClick);
		...
		case TRG_ITEMRECT:
			MakeButton(BTN_STATIC,ID_TRIG0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"If any",NULL);
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,90,y+17,140,14,GetItem(spcl.trigger[t].value)->name,ItemClick);
			MakeButton(BTN_STATIC,ID_TRIG0+OFS_CUSTOM+2+100*t,0,235,y+17,1,1,"are in the area: ",NULL);
			sprintf(s,"(%d,%d)-(%d,%d)",spcl.trigger[t].x,spcl.trigger[t].y,((word)spcl.trigger[t].value2)%256,((word)spcl.trigger[t].value2)/256);
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+3+100*t,0,370,y+17,150,14,s,RectClick);
			break;
		...
		case TRG_MONSCOLOR:
			...
			s[1]='\0';
			s[0]=spcl.trigger[t].value2;
			MakeButton(BTN_COLOR,ID_TRIG0+OFS_CUSTOM+5+100*t,0,390,y+17,14,14,s,ColorClick1);
			break;
		case TRG_EQUATION:
			MakeButton(BTN_STATIC,ID_TRIG0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"If",NULL);
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+3+100*t,0,60,y+17,250,14,spcl.effect[t].text,EquationClick);
		...
		case TRG_EQUVAR:
			...
			MakeButton(BTN_NORMAL,ID_TRIG0+OFS_CUSTOM+1+100*t,0,363,y+17,30,14,VarName(spcl.trigger[t].value),VarClick);
...
static void SetupEffectButtons(int t,int y)
{
	...
	switch(spcl.effect[t].type)
	{
		...
		case EFF_MESSAGE:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,160,y+17,250,14,spcl.effect[t].text,MessageClick);
			if(spcl.effect[t].flags&EF_NOFX)
		...
		case EFF_SOUND:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,150,y+17,250,14,GetSoundInfo(spcl.effect[t].value)->name,SoundClick);
			break;
		case EFF_SONG:
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"Play song",NULL);
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,140,y+17,150,14,spcl.effect[t].text,SongClick);
			break;
		case EFF_WINLEVEL:
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"Win level and go to",NULL);
			sprintf(s,"%s",EditorGetWorld()->map[spcl.effect[t].value]->name);
			...
			if(spcl.effect[t].x==255)
				strcpy(s,"AUTO");
			else
				sprintf(s,"%d, %d",spcl.effect[t].x,spcl.effect[t].y);
		...
		case EFF_TELEPORT:
			...
			if(spcl.effect[t].flags&EF_PLAYER)
				strcpy(s,"Player");
			else if(spcl.effect[t].flags&EF_TAGGED)
				strcpy(s,"Tagged");
			else
				strcpy(s,"Target");
		...
		case EFF_CHANGETILE:
			...
			sprintf(s,"%c%c",(char)(spcl.effect[t].value%128),(char)(spcl.effect[t].value/128));
			...
			sprintf(s,"%c%c",(char)(spcl.effect[t].value2%128),(char)(spcl.effect[t].value2/128));
		...
		case EFF_SUMMON:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,104,y+17,140,14,MonsterName(spcl.effect[t].value),MonsterClick);
			...
			if(spcl.effect[t].value2==ITM_RANDOM)
				MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+5+100*t,0,375,y+17,120,14,"Random",MonsItemClick);
			else
				MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+5+100*t,0,375,y+17,120,14,GetItem(spcl.effect[t].value2)->name,MonsItemClick);
		...
		case EFF_LIGHT:
			...
			if(spcl.effect[t].flags&EF_PERMLIGHT)
			...
			sprintf(s,"%d",spcl.effect[t].value);
			...
			sprintf(s,"%d",spcl.effect[t].value2);
		...
		case EFF_PICTURE:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,140,y+17,250,14,spcl.effect[t].text,PicNameClick);
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+2+100*t,0,394,y+17,1,1,"mode",NULL);
			switch(spcl.effect[t].value)
			{
		...
		case EFF_ITEM:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+5+100*t,0,336,y+17,120,14,GetItem(spcl.effect[t].value)->name,Item2Click);
			if(spcl.effect[t].flags&EF_NOFX)
				MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+4+100*t,0,520,y+17,65,14,"No FX",NoFXClick);
			else
				MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+4+100*t,0,520,y+17,65,14,"Play FX",NoFXClick);
		...
		case EFF_SWAPMAP:
			...
			sprintf(s,"(%d,%d)-(%d,%d)",spcl.effect[t].x,spcl.effect[t].y,((word)spcl.effect[t].value)%256,((word)spcl.effect[t].value)/256);
			...
			sprintf(s,"(%d,%d)-(%d,%d)",((word)spcl.effect[t].value2)%256,((word)spcl.effect[t].value2)/256,
				(((word)spcl.effect[t].value2)%256)+((((word)spcl.effect[t].value)%256)-spcl.effect[t].x),
				(((word)spcl.effect[t].value2)/256)+((((word)spcl.effect[t].value)/256)-spcl.effect[t].y)
				);
		...
		case EFF_CHANGEMONS:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+5+100*t,0,360,y+17,140,14,MonsterName(spcl.effect[t].value2),Monster2Click);
		...
		case EFF_VAR:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,130,y+17,30,14,VarName(spcl.effect[t].value),VarClick);
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+2+100*t,0,164,y+17,1,1,"to",NULL);
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+3+100*t,0,184,y+17,250,14,spcl.effect[t].text,EquationClick);
			break;
		case EFF_LIGHTRECT:
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"Make",NULL);
			if(spcl.effect[t].flags&EF_PERMLIGHT)
		...
		case EFF_LEVELFLAG:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,170,y+17,140,14,lvlFlagName[spcl.effect[t].value],LevelFlagClick);
		...
		case EFF_LIFE:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,200,y+17,140,14,MonsterName(spcl.effect[t].value),MonsterClick);
		...
		case EFF_WEAPON:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,224,y+17,140,14,WeaponName(spcl.effect[t].value),WeaponClick);
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+2+100*t,0,370,y+17,1,1,"and",NULL);

			if(spcl.effect[t].value2==0)
		...
		case EFF_MONSITEM:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,84,y+17,140,14,MonsterName(spcl.effect[t].value),MonsterClick);
			...
			if(spcl.effect[t].value2==ITM_RANDOM)
				...
				MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+5+100*t,0,390,y+17,110,14,GetItem(spcl.effect[t].value2)->name,MonsItemClick);
		...
		case EFF_TILEVAR:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,384,y+17,30,14,VarName(spcl.effect[t].value),VarClick);
		...
		case EFF_LIFEAMT:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,124,y+17,140,14,MonsterName(spcl.effect[t].value),MonsterClick);
		...
		case EFF_AI:
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"Change",NULL);
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,94,y+17,140,14,MonsterName(spcl.effect[t].value),MonsterClick);
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+5+100*t,0,375,y+17,140,14,MonsterName(spcl.effect[t].value2),Monster2Click);
		...
		case EFF_NAME:
			...
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+5+100*t,0,405,y+17,140,14,spcl.effect[t].text,MessageClick);
			break;
		case EFF_COLOR:
			MakeButton(BTN_STATIC,ID_EFF0+OFS_CUSTOM+0+100*t,0,40,y+17,1,1,"Change",NULL);
			MakeButton(BTN_NORMAL,ID_EFF0+OFS_CUSTOM+1+100*t,0,94,y+17,140,14,MonsterName(spcl.effect[t].value),MonsterClick);
...
void SpecialEdit_Update(int mouseX,int mouseY,MGLDraw *mgl)
{
	switch(mode)
	{
			...
			if(TextDialogCommand()!=0)
			{
				if(TextDialogCommand()==TM_OKAY)
				{
					if(mode==SMODE_MESSAGE)
					{
						...
						SetupEffectButtons(curEff,curEff*38+264);
						SetupTriggerButtons(curEff,curEff*38+30);
		...
		case SMODE_PICKTRIG:
			if(mgl->MouseDown())
			{
				if((mouseY-selectY)/14>=MAX_TRIGGER)
					spcl.trigger[curTrig].type=TRG_NONE;
				else
					spcl.trigger[curTrig].type=(mouseY-selectY)/14;
			}
			else
			{
				if((mouseY-selectY)/14>=MAX_TRIGGER)
