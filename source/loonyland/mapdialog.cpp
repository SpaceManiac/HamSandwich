...
#define MAPS_PER_PAGE	24

char mapnames[MAX_MAPNAMES][40];
char newmapname[34]="";
static byte mapNum,topMap;
world_t *world;
...
void InitMapDialog(world_t *wrld,byte currentMap)
{
	...
	for(i=0;i<MAX_MAPNAMES;i++)
		mapnames[i][0]='\0';
	i=0;
	topMap=MAPS_PER_PAGE*(currentMap/MAPS_PER_PAGE);
...
void RenderMapDialog(int msx,int msy,MGLDraw *mgl)
{
	...
	{
		if(i+topMap<MAX_MAPNAMES)
			Print(67,16+i*14,mapnames[i+topMap],0,1);
	...
	mgl->Box(380,320,470,320+14,16);
	Print(382,322,"NextPage",0,1);
}
...
byte MapDialogClick(int msx,int msy)
{
	...
	for(i=0;i<MAPS_PER_PAGE;i++)
		if(msx>64 && msx<362 && msy>13+i*14 && msy<14+(i+1)*14)
		{
			if(i+topMap<world->numMaps)
			{
				EditorSelectMap(i+topMap);
				mapNum=i+topMap;
			}
		}
	...
	if(msx>379 && msy>11 && msx<471 && msy<11+15)
	{
		...
		{
			world->numMaps++;
			world->map[world->numMaps-1]=new Map(25,25,newmapname);
		}
	}
	if(msx>379 && msy>28 && msx<471 && msy<28+15)
	{
		...
		{
			world->numMaps++;
			world->map[world->numMaps-1]=new Map(64,64,newmapname);
		}
	}
	if(msx>379 && msy>44 && msx<471 && msy<44+15)
	{
		...
		{
			world->numMaps++;
			world->map[world->numMaps-1]=new Map(128,128,newmapname);
		}
	}
	...
	if(msx>379 && msy>320 && msx<471 && msy<320+15)
	{
		topMap+=MAPS_PER_PAGE;
		if(topMap>=world->numMaps)
			topMap=0;
	}
