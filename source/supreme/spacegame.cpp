...
#define SPU_BOMB		0
#define SPU_RAPID		1
#define SPU_EXPLODE		2
#define SPU_TRIPLE		3
#define SPU_REAR		4
#define SPU_SHIELD		5
#define SPU_WINGMAN		6
#define SPU_LIFE		7
...
static int addTime,coinsEarned;
...
void InitSpaceGame(MGLDraw *mgl)
{
	...
	for(i=0;i<8;i++)
		spcPlyr.powerup[i]=0;
	spcPlyr.powerup[SPU_SHIELD]=200;
	spcPlyr.combo=0;
...
void BadguyHit(int me)
{
	...
	if(spcBadguy[me].hp==0)
	{
		...
		{
			case SG_POWERUP:
				MakeSpaceSound(SND_MISSILEBOOM,800);
				for(i=0;i<8;i++)
			...
			case SG_MINI:
				...
				break;
			case SG_MAXI:
				MakeSpaceSound(SND_BOMBBOOM,800);
				for(i=0;i<32;i++)
		...
		MakeSpaceSound(SND_ROBOOUCH,400);
}
...
void BulletPop(int me)
{
	...
	switch(spcBullet[me].type)
	{
		...
		case SB_PBULLET2:
			...
			if(spcPlyr.powerup[SPU_EXPLODE] && spcBullet[me].type==SB_PBULLET)
			{
				for(i=0;i<4;i++)
				{
...
byte SpaceUpdatePlayerShip(MGLDraw *mgl)
{
	...
	if((c&CONTROL_B2) && spcPlyr.reload2==0 && spcPlyr.bombs)
	{
		...
		spcPlyr.reload2=15;
		spcPlyr.invinc=60;
		MakeSpaceSound(SND_BOMBBOOM,2000);
		for(i=0;i<256;i++)
	...
	for(i=0;i<8;i++)
	{
		if(spcPlyr.powerup[i] && i!=SPU_SHIELD)
		{
			spcPlyr.powerup[i]--;
			if(i==SPU_WINGMAN && spcPlyr.powerup[i]==0)
				RemoveWingman();
		}
	}
	oldc=c;
...
void SpacePowerUp(byte type)
{
	...
	MakeSpaceSound(SND_CANDLEGET,500);
	switch(type)
	{
		case SPU_BOMB:
...
void SpaceGoodguyHit(void)
{
	...
	if(spcPlyr.powerup[SPU_SHIELD])
	{
		...
		spcPlyr.powerup[SPU_SHIELD]/=2;
		if(spcPlyr.powerup[SPU_SHIELD]<100)
			spcPlyr.powerup[SPU_SHIELD]=0;
		return;
	}
	else
	{
		MakeSpaceSound(SND_BOMBBOOM,5000);
		for(i=0;i<12;i++)
...
void SpaceUpdateBadguys(byte evilUp)
{
	int i,p,a;
			...
			switch(spcBadguy[i].type)
			{
				case SG_UFO:
					...
					if(spcBadguy[i].timer==0)
					{
						...
						spcBadguy[i].timer=60;
						MakeSpaceSound(SND_ROBOSHOOT,300);
				...
				case SG_MOTHER:
					...
					if(spcBadguy[i].timer==0)
					{
						...
						MakeSpaceSound(SND_MUSHMISSILE,1000);
				...
				case SG_POWERUP:
					...
					Clamp(&spcBadguy[i].dx,FIXAMT*6);
					Clamp(&spcBadguy[i].dy,FIXAMT*6);
					if(CheckRect(spcBadguy[i].x-FIXAMT*10,spcBadguy[i].y-FIXAMT*10,
								 spcBadguy[i].x+FIXAMT*10,spcBadguy[i].y+FIXAMT*10,
								 spcPlyr.x-FIXAMT*10,spcPlyr.y-FIXAMT*10,
								 spcPlyr.x+FIXAMT*10,spcPlyr.y+FIXAMT*10))
		...
		if(numBadguys==100)
		{
			for(i=0;i<NUMBADGUYS;i++)
			{
				if(spcBadguy[i].type==SG_NONE)
				{
					...
					PutOnEdge(i);
					spcBadguy[i].timer=30;
					MakeSpaceSound(SND_GNOMELAUGH,8000);
					return;
				}
			}
			return;
