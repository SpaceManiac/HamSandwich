...
void Scan_Trigger(world_t *world,Map *map,int num,trigger_t *me,char *effText)
{
	...
	switch(me->type)
	{
		...
		case TRG_ITEMRECT:
			...
			break;
		case TRG_DIFFICULTY:
			SDL_IOprintf(scanF,"If difficulty is %s", GetDifficultyName(me->value));
			PrintLessMore(me->flags);
			break;
		case TRG_KEYPRESS:
			...
			break;
		case TRG_PLAYAS:
			SDL_IOprintf(scanF,"If player is playing as %s", GetPlayableCharacterName(me->value));
			break;
		case TRG_MONSCOLOR:
		...
		case TRG_EQUVAR:
			...
			break;
		case TRG_BULLETRECT:
			SDL_IOprintf(scanF,"If any %s are in (%03d,%03d)-(%03d,%03d)",BulletName(me->value),me->x,me->y,((word)me->value2)%256,((word)me->value2)/256);
			break;
	}
	if(me->flags&TF_AND)
...
void Scan_Effect(world_t *world,Map *map,int num,effect_t *me)
{
	...
	switch(me->type)
	{
		...
		case EFF_LIGHTRECT:
			...
			break;
		case EFF_LEVELFLAG:
			SDL_IOprintf(scanF,"Change level flag %s to ",MapFlagName(me->value));
			switch(me->value2)
			{
				case 0:
		...
		case EFF_LIFE:
			...
			break;
		case EFF_WEAPON:
			SDL_IOprintf(scanF,"Force player's weapon to %s and ",GetWeaponName(me->value));
			if(me->value2==0)
		...
		case EFF_MONSBRIGHT:
			...
			break;
		case EFF_PLAYAS:
			SDL_IOprintf(scanF,"Force player to play as %s\n", GetPlayableCharacterName(me->value));
			break;
