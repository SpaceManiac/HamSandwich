...
void TryToPush(Guy *me,int x,int y,Map *map,world_t *world)
{
	...
	if(GetTerrain(world,tile->floor)->flags&TF_PUSHON)
	{
		...
		for(i=0;i<maxGuys;i++)
			if((guys[i]) && (guys[i]->type) && !(MonsterFlags(guys[i]->type,guys[i]->aiType)&MF_NOHIT) && (guys[i]->hp>0) &&
...
byte Walkable(Guy *me,int x,int y,Map *map,world_t *world)
{
	...
	if((cheatsOn&1) && me->type==MONS_BOUAPHA)
		return 1;
...
{
	if((seq==ANIM_DIE) || (seq==ANIM_A3 && (aiType==MONS_BOUAPHA || aiType==MONS_GOODBONE || aiType==MONS_REFLECTION)))
	{
		...
		if(ignited)
		{
			FireExactBullet(x,y,0,0,0,0,0,ignited,0,BLT_SITFLAME,1);
		}
	}
	seq=ANIM_IDLE;
	frm=0;
	if(type!=MONS_SCAREDYBAT)
		frmAdvance=128;
	action=ACTION_IDLE;
}
...
void Guy::Update(Map *map,world_t *world)
{
	...
	if(poison>0)
	{
		if(hp>0 && ((poison&3)==0))
		{
			...
			else
			{
				...
				{
					hp=1;
					player.shield=0;	// can't be protected by lightreaver
	...
	if(ignited)
	{
		...
		if((ignited%(15-(player.weaponLvl[WPN_FLAME-1])))==0)
		{
			GetShot(0,0,1,map,world);
			if(WorthXP(type))
	...
	if(!CanWalk(x,y,map,world))
	{
		...
		if(aiType==MONS_ROLLER || aiType==MONS_ROLLER2 ||
			aiType==MONS_BOUAPHA || aiType==MONS_VAMPIRE || aiType==MONS_MADCOW || aiType==MONS_POLTERGUY || aiType==MONS_POLTERGUY2 ||
			aiType==MONS_BUNNY || aiType==MONS_LOONYSHIP || aiType==MONS_FRIENDLY || aiType==MONS_SNOWBALL ||
			aiType==MONS_SNOWBALL2 || aiType==MONS_ROLLER3 || aiType==MONS_ROLLER4 || aiType==MONS_RADISH || aiType==MONS_PUMPKINFLY2 ||
			aiType==MONS_FRIENDLY2 || aiType==MONS_CRAZYPANTS || aiType==MONS_YUGO || aiType==MONS_SPEEDY || aiType==MONS_SPARKY ||
			aiType==MONS_PATTY || aiType==MONS_PATROLLR || aiType==MONS_PATROLUD || aiType==MONS_DPATROLLR || aiType==MONS_DPATROLUD ||
			aiType==MONS_MINIWACKO || aiType==MONS_JACKFROST || aiType==MONS_BIGHEAD2)
	...
	if(aiType==MONS_BOUAPHA)	// special case, Bouapha is the player, follow him
	{
		...
		if((hp>0) && (z==0) && (GetTerrain(world,map->GetTile(mapx,mapy)->floor)->flags&TF_LAVA)
			&& (!PlayerCanWaterwalk()) && (!(player.vehicle==VE_RAFT)) && (!(player.vehicle==VE_YUGO))
		...
		// standing on poison stuff
		if((hp>0) && (z==0) && (GetTerrain(world,map->GetTile(mapx,mapy)->floor)->flags&TF_MUD)
			&& (!PlayerCanWaterwalk()) && (!(player.vehicle==VE_RAFT)) && (!(player.vehicle==VE_YUGO))
			 && ((MonsterFlags(goodguy->type,goodguy->aiType)&(MF_AQUATIC|MF_FLYING))==0))
		{
			if(!RaftNearby())
			{
				if(poison==0)
					MakeSound(SND_THINGVOMIT,x,y,SND_CUTOFF,400);
				poison=32;
			}
		}
	...
	if((oldmapx!=mapx || oldmapy!=mapy) && type!=MONS_NOBODY && hp>0)
	{
		if(mapx==63 && mapy==53 && type==MONS_BOUAPHA && player.levelNum==12)
		{
			...
			{
				player.journal[61]=1;
				map->map[64+55*map->width].item=ITM_KEY;
			}
		}
...
void Guy::MonsterControl(Map *map,world_t *world)
{
	...
	if(aiType==MONS_BURNER || aiType==MONS_ROLLER || aiType==MONS_ROLLER2 || aiType==MONS_COFFIN || aiType==MONS_DARKCOFFIN ||
		aiType==MONS_BURNER2 || aiType==MONS_ALIENEGG2 || aiType==MONS_EGGSAC || aiType==MONS_BOILER ||
	...
	if(aiType!=255 && GetMonsterType(aiType)->AI)
...
void Guy::GetShot(int dx,int dy,byte damage,Map *map,world_t *world)
{
	...
	if((cheatsOn&2) && type==MONS_BOUAPHA)
		return;

	t=type;

	if(type==MONS_STICKTREE && GetBulletAttackType()==BLT_ENERGY)
		return;	// immune!
	...
	if(GetBulletAttackType()!=BLT_LASER && (type==MONS_TURRETU || type==MONS_TURRETL || type==MONS_TURRETD || type==MONS_TURRETR))
		return;	// immune to all but lasers
	if(GetBulletAttackType()==BLT_LASER && (type==MONS_TURRETU || type==MONS_TURRETL || type==MONS_TURRETD || type==MONS_TURRETR))
	{
		if(player.weaponLvl[WPN_REFLECT-1]==0)
			return;	// can't be harmed at all until you have reflecto powers
	}
	...
	if(aiType==MONS_BOUAPHA && PlayerShield() && damage!=255)
	...
	if(type==MONS_GRUE || type==MONS_POLTERGUY || type==MONS_RAFE || type==MONS_SPEEDY || type==MONS_COUNTESS || type==MONS_SKULL)	// bosses drop ammo when hit
	{
		if(Random(100)<20)
			FireBullet(x,y,Random(256),BLT_AMMOBLIP,1);
	}

	if(aiType==MONS_RAFE && GetBulletAttackType()!=BLT_NONE)
	{
		return;	// immune to bullets
	}

	if(aiType==MONS_BOUAPHA && player.ability[ABIL_SHIELD] && Random(10)==0 && damage!=255)
	{
		MakeNormalSound(SND_CLANG);	// deflect
		return;
	}
	formerHP=hp;
	ouch=4;

	if(aiType==MONS_BOUAPHA && map->flags&MAP_LAVA)
	{
		if(player.ammo-damage<0)
			player.ammo=0;
		else
			player.ammo-=damage;
	}
	...
	{
		if(GetBulletAttackType()==BLT_BLACKHOLE)
			PlayerGetWeaponXP(WPN_PORTAL,realDam);
		if(GetBulletAttackType()==BLT_BIGAXE)
			PlayerGetWeaponXP(WPN_SONIC,realDam);
		if(GetBulletAttackType()==BLT_SCANSHOT)
			PlayerGetWeaponXP(WPN_SPARKS,realDam);
		if(GetBulletAttackType()==BLT_SITFLAME)
			PlayerGetWeaponXP(WPN_FLAME,realDam);
		if(GetBulletAttackType()==BLT_REFLECT)
			PlayerGetWeaponXP(WPN_REFLECT,realDam);
		if(GetBulletAttackType()==BLT_GREEN || (GetBulletAttackType()==0 && !friendly && !ignited))
			PlayerGetWeaponXP(WPN_BONEHEAD,realDam);
	}

	if(hp<=0)
	{
		if(type==MONS_SERPENT && player.journal[55]<100)	// aquazoids are worth fishing XP
			player.journal[55]++;

		if(type==MONS_POLTERGUY && map->flags&MAP_LAVA)	// can't die on The Dumb Side, just ghost out with 1 hp
		{
			hp=1;
			type=MONS_POLTERGUY2;
			ignited=0;
			MakeSound(SND_SPOOKSHIFT,x,y,SND_CUTOFF,2000);
			map->BrightTorch(mapx,mapy,16,6);
			return;
		}
		if(friendly==0)
		{
			if(WorthXP(type) && GetBulletAttackType()!=BLT_LASER)
			{
				PlayerGetXP(maxHP);
				for(i=0;i<maxHP;i++)
				{
					if(Random(100)<10)
						FireBullet(x,y,Random(256),BLT_LIFEBLIP,1);
					if(Random(100)<20)
						FireBullet(x,y,Random(256),BLT_AMMOBLIP,1);
				}
			}
		...
		frmTimer=0;
		action=ACTION_BUSY;
		if(type==MONS_SVEN || type==MONS_SHOCKTR ||
...
void UpdateGuys(Map *map,world_t *world)
{
	...
	if(player.ability[ABIL_BRAIN] && goodguy)
	{
		if(goodguy->dx==0 && goodguy->dy==0)	// only works when standing still
		{
			if(player.brainTime)
				player.brainTime--;
			else
			{
				...
				{
					player.brainX=goodguy->mapx;
					player.brainY=goodguy->mapy;
				}
	...
	if(player.ability[ABIL_SOLAR] && !(map->flags&MAP_LAVA))
	{
		if((player.clock%30)==0 && player.ammo<100)
			player.ammo++;
	}
	if(player.ability[ABIL_FIRSTAID])
	{
		if((player.clock%(30*3))==0)
			HealGoodguy(1);
	}
	...
	if(map->flags&MAP_LAVA)
	{
		if(player.vehicle!=VE_YUGO)
		{
			player.lavaTimer--;
			if(!player.lavaTimer)
			{
				player.lavaTimer=2+player.weaponLvl[WPN_PORTAL-1]*2;
				if(player.ammo)
				{
					player.ammo--;
					PlayerGetWeaponXP(WPN_PORTAL,1);
				}
				else
				{
					SetPlayerStart(-2,-2);
					SendMessageToGame(MSG_DUMBDIE,player.mapFrom);
					//GetSpook();
					//LoadState(player.mapFrom,2);
					//PutSpook();
				}
			}
		}
	...
	for(i=0;i<maxGuys;i++)
		if(guys[i]->type==MONS_NONE)
		{
			...
			if(friendly==1 || (friendly==2 && (type==MONS_BOUAPHA || type==MONS_FRIENDLY || type==MONS_GOODTURRET ||
				type==MONS_GOODBONE || type==MONS_GOODROBOT || type==MONS_GOODROBOT2 ||
				type==MONS_FRIENDLY2 || type==MONS_FOLLOWBUNNY || type==MONS_MINECART || type==MONS_RAFT ||
				type==MONS_YUGO || type==MONS_PUNKBUNNY || type==MONS_REFLECTION || type==MONS_SCAREDYBAT)))
			{
				guys[i]->friendly=1;
		...
		if(map->badguy[i].type)
		{
			...
			if(g)
			{
				...
				if(g->type==MONS_BOUAPHA)
				{
					g->item=ITM_NONE;
					PutPlayerAtStart(g);
//					g=AddGuy(g->x,g->y,0,MONS_KITTEN);
				}
			}
		}
...
// Find Victims that has a rnadom chance of hitting depending on Lightreaver level
byte FindVictimsRand(int x,int y,byte size,int dx,int dy,byte damage,Map *map,world_t *world,byte friendly)
{
	...
	for(i=0;i<maxGuys;i++)
		if(guys[i]->type && guys[i]->hp && (guys[i]->friendly!=friendly) && Random(10)<player.weaponLvl[WPN_REFLECT])
		{
			if(CheckHit(size,x,y,guys[i]))
			{
				guys[i]->GetShot(dx,dy,damage,map,world);
...
word LockOnEvil(int x,int y)
{
	...
	for(i=0;i<maxGuys;i++)
		...
		{
			if(guys[i]->type==MONS_LIGHTSWITCH && guys[i]->seq==ANIM_ATTACK)
				continue;
...
void KillMonster(int x,int y,int type,byte nofx)
{
	...
	for(i=0;i<maxGuys;i++)
	{
		if(guys[i]->type!=MONS_NONE && guys[i]->type!=MONS_NOBODY && (x==255 || (guys[i]->mapx==x && guys[i]->mapy==y)))
		{
			...
			for(j=0;j<maxGuys;j++)
				if(guys[j]->parent==guys[i])
				{
					guys[j]->type=MONS_NONE;
				}
...
void KillMonsterRect(int x,int y,int x2,int y2,int type,byte nofx)
{
	...
	for(i=0;i<maxGuys;i++)
	{
		if(guys[i]->type!=MONS_NONE && guys[i]->type!=MONS_NOBODY && (guys[i]->mapx>=x && guys[i]->mapy>=y && guys[i]->mapx<=x2 && guys[i]->mapy<=y2))
		{
			switch(type)
			{
				case MONS_ANYBODY:
...
byte CheckMonsterLife(int x,int y,int type,int life,byte flags)
{
	...
	for(i=0;i<maxGuys;i++)
	{
		if(guys[i]->type!=MONS_NONE && guys[i]->type!=MONS_NOBODY && (x==255 || (guys[i]->mapx==x && guys[i]->mapy==y)))
		{
			...
			if(life==255 && guys[i]->hp==guys[i]->maxHP)
				return 1;
			if(guys[i]->hp==life)
				return 1;
...
void ChangeMonsItem(byte fx,int x,int y,int type,int newItem)
{
	...
	for(i=0;i<maxGuys;i++)
	{
		if(guys[i]->type!=MONS_NONE && guys[i]->type!=MONS_NOBODY && (x==255 || (guys[i]->mapx==x && guys[i]->mapy==y)))
		{
			...
			if(guys[i]->aiType==MONS_BOUAPHA || guys[i]->aiType==MONS_GOODBONE)
			{
...
void FindMonsterBrain(int myx,int myy)
{
	...
	for(i=0;i<maxGuys;i++)
	{
		if(guys[i]->type && guys[i]->hp && guys[i]->aiType!=MONS_BOUAPHA)
		{
			if(guys[i]->item<ITM_RANDOM && GetItem(guys[i]->item)->effect==IE_BRAIN &&
...
byte GuyIsAlive(byte spawnNum)
{
	int i;

	for(i=0;i<maxGuys;i++)
		if(guys[i]->spawnNum==spawnNum && guys[i]->hp && guys[i]->type)
			return 1;
	return 0;
}
...
void RespawnGuys(Map *map)
{
	...
	for(i=0;i<MAX_MAPMONS;i++)
	{
		if(DoesRespawn(map->badguy[i].type) && !GuyIsAlive((byte)i) && Random(100)==0)
		{
			if(abs(goodguy->mapx-map->badguy[i].x)+abs(goodguy->mapy-map->badguy[i].y)>16)
			{
...
byte EatHay(Guy *me,byte rad)
{
	...
	for(i=me->mapx-rad;i<=me->mapx+rad;i++)
		...
		{
			if(i>=0 && j>=0 && i<curMap->width && j<curMap->height && curMap->map[i+j*curMap->width].item==151)
			{
...
void EasyStart(void)
{
	...
	for(i=0;i<maxGuys;i++)
	{
		if(guys[i]->type && (guys[i]->item==ITM_NONE || guys[i]->item==ITM_RANDOM) && DoesRespawn(guys[i]->type) && guys[i]->friendly==0 && PointInRect(guys[i]->x,guys[i]->y,goodguy->x-32*2*FIXAMT,goodguy->y-24*2*FIXAMT,
																goodguy->x+32*2*FIXAMT,goodguy->y+24*2*FIXAMT))
		{
			guys[i]->type=MONS_NONE;
		}
	}
...
void SuckInEvil(int x,int y)
{
	...
	for(i=0;i<maxGuys;i++)
	{
		if(guys[i]->type && guys[i]->friendly==0 && guys[i]->hp && !(MonsterFlags(guys[i]->type,0)&MF_NOMOVE))
		{
...
void PutSpook(void)
{
	...
	if(holdSpook!=NULL)
	{
		...
		g->target=goodguy;
		g->parent=NULL;
		g->type=MONS_POLTERGUY+MONS_POLTERGUY2-g->type;
		delete holdSpook;
		holdSpook=NULL;
	}
}
...
byte Mind1At(int x,int y)
{
	...
	for (i=0;i<maxGuys;i++)
	{
		if(guys[i]->mapx==x && guys[i]->mapy==y)
			return guys[i]->mind1;
	}
	return 255;
}
