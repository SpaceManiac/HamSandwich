...
int JamulSoundCreateBuffer(int bufferLen, WAVEFORMATEX wfmtx)
{
	...
	dsbdesc.dwFlags = DSBCAPS_CTRLPAN|DSBCAPS_CTRLVOLUME|DSBCAPS_STATIC;
	...
	soundbuf[nextOpenBuffer].size=bufferLen;
...
bool JamulSoundPlay(int which,long pan,long vol,byte playFlags)
{
	...
	{
		hr = soundbuf[which].buf->Restore();
		if (SUCCEEDED(hr))
		{
			hr = soundbuf[which].buf->Play(0, 0, flags);
		}
	}
	*/
	return TRUE;
...
void JamulSoundUpdate(void)
{
	...
	{
		// hack to make the intro song play once and then switch to looping beatnik song
		if(playBuffer[0].soundNum==205 && (!JamulSoundIsPlaying(playBuffer[0].dsHandle)))
		{
			JamulSoundSong(204,-700);
		}
	}
}
...
void JamulSoundSong(int num,long vol)
{
	...
	if(num!=205)	// hack for the intro song to not loop
		JamulSoundPlay(playBuffer[0].dsHandle,playBuffer[0].pan,playBuffer[0].vol,SOUND_LOOP);
	else
...
void JamulSoundPurge(void)
{
	...
	for(i=1;i<MAX_SOUNDS_AT_ONCE;i++)
	{
		JamulSoundDestroyBuffer(playBuffer[i].dsHandle);
		playBuffer[i].dsHandle=-1;
		playBuffer[i].flags=0;
		playBuffer[i].soundNum=-1;
	}
	...
	for(i=0;i<dsoundBufferCount;i++)
	{
		if(soundHandle[i]!=-1)
		{
			// don't destroy if it is the currently playing song
			if(playBuffer[0].dsHandle==-1 || playBuffer[0].soundNum!=i)
			{
